...
void saveTF(const tf::StampedTransform& transform, const std::string& filepath, int cam_num)
{
  ...
  calib_file << "# This file was generated by kuka_calib\n# Transform between the world frame to the camera link\n# Position\n"
             << "export cam_" << cam_num << "_x_pos=" << transform.getOrigin()[0] << "\n"
             << "export cam_" << cam_num << "_y_pos=" << transform.getOrigin()[1] << "\n"
             << "export cam_" << cam_num << "_z_pos=" << transform.getOrigin()[2] << "\nRotation\n"
             << "export cam_" << cam_num << "_x_rot=" << transform.getRotation().x() << "\n"
             << "export cam_" << cam_num << "_y_rot=" << transform.getRotation().y() << "\n"
             << "export cam_" << cam_num << "_z_rot=" << transform.getRotation().z() << "\n"
             << "export cam_" << cam_num << "_w_rot=" << transform.getRotation().w() << "\n";
}
...
int main(int argc, char** argv)
{
  ...
  // Create a unique filename through timestamp
  auto t = std::time(nullptr);
  auto tm = *std::localtime(&t);
  std::ostringstream oss;
  oss << std::put_time(&tm, "%Y_%m_%d_%H_%M_%S");
  std::string timestamp = oss.str();
  std::string calib_file = calib_dir + '/' + timestamp + "_cam_" + std::to_string(cam_num) + "_calib.sh";
  ...
  saveTF(world2camlink, calib_file, cam_num);
